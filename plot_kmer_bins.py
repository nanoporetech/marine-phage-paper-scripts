...
def parse_args():
    ...
    # Optional arguments
    parser.add_argument("-p", "--prefix", help="Output file prefix [bins]", type=str, default="bins")
...
def scatterplot(df, title, args):
    ...
    with open("{}.bin_coords.tsv".format(args.prefix), "w") as f:
        f.write("{}\t{}\t{}\n".format("bin", "x", "y"))
        for bin_id in range(-1,max(df["bin_id"])+1):
    ...
    plot_fn = "{}.hdbscan_bins.png".format(args.prefix)
